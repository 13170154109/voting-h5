...
class PhotosController extends Controller
{
    ...
    public function actionUpload($folderSlug)
    {
        $success = null;

        $photo = new Photo;

        $photo->class = Category::className();
        $photo->item_id = 1;

        $cat = Category::findOne(["slug" => $folderSlug]);
        if ($cat) {
            $photo->item_id = $cat->category_id;
        }
        ...
        if($photo->image && $photo->validate(['image'])){
            ...
            if($photo->image){
                ...
                if(file_exists(($filename = Yii::getAlias('@webroot') . $photo->image)))
                {
                ...
                if($photo->save()){
                    ...
                    $this->error = Yii::t('easyii', 'Create error. {0}', $carousel->formatErrors());
                }
            }
            else{
    ...
    public function actionDelete($id)
    {
        ...
        return $this->formatResponse(Yii::t('easyii', 'Carousel deleted'));
    }
