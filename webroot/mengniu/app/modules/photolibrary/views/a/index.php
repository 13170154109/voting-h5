    ...
    <table class="table table-hover font-12 ">
        <tbody>
            <?php foreach($cats as $cat) : ?>
                <tr>
                    <td width="50"><i class="glyphicon glyphicon-folder-open font-12"></i></td>
                    <td style="padding-left:  <?= $cat->depth * 20 ?>px;">
                        ...
                        <?php if(!count($cat->children) || !empty(Yii::$app->controller->module->settings['itemsInFolder'])) : ?>
                            <a href="<?= Url::to([$baseUrl . $this->context->viewRoute, 'id' => $cat->category_id, 'select'=>$select, 'width'=>$width, 'height'=>$height]) ?>" class="<?= ($cat->status == CategoryModel::STATUS_OFF ? 'smooth"' : '') ?>"><?= $cat->title ?></a>
                        <?php else : ?>
                    ...
                    <td width="120" class="text-right">
                        <div class="dropdown actions">
                            ...
                            <ul class="dropdown-menu dropdown-menu-right" role="menu" aria-labelledby="dropdownMenu<?= $cat->category_id ?>">
                                <li><a href="<?= Url::to([$baseUrl.'/a/edit', 'id' => $cat->category_id, 'select'=>$select]) ?>"><i class="glyphicon glyphicon-pencil font-12"></i> <?= Yii::t('easyii', 'Edit') ?></a></li>
